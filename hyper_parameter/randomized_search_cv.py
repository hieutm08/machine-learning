...
knn_param_grid = {'n_neighbors': np.arange(1,21), 'weights': ['uniform', 'distance']}

decision_tree_model = DecisionTreeClassifier()
decision_param_model = {'max_depth':[None] + list(np.arange(1,31)), 'min_samples_split': list(np.arange(1,11))}

knn = RandomizedSearchCV(estimator=knn_model, param_distributions=knn_param_grid, n_iter=10,scoring="accuracy", cv=5)
dt =  RandomizedSearchCV(estimator=decision_tree_model, param_distributions=decision_param_model,n_iter=10,scoring="accuracy", cv=5)
